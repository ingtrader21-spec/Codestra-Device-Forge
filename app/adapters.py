...
def discover_apple():
    ...
    if not r["available"] or r.get("returncode") != 0:
        return []
    rows=[]
    for raw in r["output"].splitlines():
        x=raw.strip()
        # UDIDs are machine identifiers; never turn error text into a device.
        if not x or "error" in x.lower() or "unable" in x.lower() or "device list" in x.lower():
            continue
        if len(x) < 16 or any(c.isspace() for c in x):
            continue
